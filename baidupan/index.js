...
<script>
  ...
  $(document).ready(function(){
    $(".fa-file").each(function(){
      const icon = getIconClass($(this).next().text());
      if (icon !== "")
      {
...
const generate = async request => {
        ...
        const res1 = await fetch('https://pan.baidu.com/s/1' + surl,
            {
                ...
                headers: {
                    ...
                    'Cookie': 'BDUSS=; '
                        + 'STOKEN=; BDCLND=' + randsk
                }
            })
        const body = await res1.text()
        const re = /yunData.setData\(({.+)\);/;
        if (body.match(re)) {
            return JSON.parse(body.match(re)[1])
        } else {
            return 1
        }
    }
    ...
    if (json2 !== 1) {
        ...
        for (let i = 0; i < filejson.list.length; i++) {
...
const link = document.forms["form1"]["surl"].value;
if (link == null || link === "") {
document.forms["form1"]["surl"].focus();
return false;
}
const uk = link.match(/uk=(\d+)/);
const shareid = link.match(/shareid=(\d+)/);
...
</div>` + dfooter, {headers: {'Content-Type': 'text/html; charset=UTF-8'}})
        } else {
